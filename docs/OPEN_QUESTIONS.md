...
- (2026-09-22, Prompt 7B2) The Seedream/Ark request shape (image array semantics, `size` enum incl. the `adaptive` default, watermark default) is implemented from cross-validated sources because the official doc page renders client-side. Confirm the exact official parameter values on the first developer-authorized live call; corrections are single-default changes in `SeedreamImageGenerationAdapter`.
