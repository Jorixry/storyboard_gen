...
- (2026-09-22, Prompt 7B2) Seedream live-call verification progress: endpoint + auth + model id `doubao-seedream-5-0-pro-260628` verified live; `size` verified to require explicit `WIDTHxHEIGHT` (the adapter now derives it from the composition PNG). Still open: first fully successful generation (composition-preservation check on the shared fixture) once a live call succeeds, and the preset list remains undocumented.
