...
- `SeedreamImageGenerationAdapter` (`src/adapters/image-generation/seedream.ts`, id `seedream@1.0.0`, the D027 primary): POSTs `{base}/images/generations` with Bearer `ARK_API_KEY`, model default `doubao-seedream-5-0-pro-260628` (the developer's activated Ark-console model, 2026-09-22; overridable via `SEEDREAM_MODEL`), images as data URLs with the composition FIRST (references then style follow), `response_format: b64_json`. Base URL, model and `size` (default `adaptive`, overridable via `SEEDREAM_SIZE`) are constructor/env overridable. ⚠️ The Ark request-shape details are cross-validated (official doc page renders client-side; see docs/PROVIDER_SPIKE.md UNCONFIRMED items) and must be confirmed on the first authorized live call; the account-side path was already live-verified (endpoint + auth + structured `ModelNotOpen` error surfaced through the full 7B1 error contract on 2026-09-22).
