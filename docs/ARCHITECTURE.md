...
## Enhanced first-frame service (Prompt 7B1 / Phase 2 Day 8 fallback)

Implemented 2026-09-22 as the mock-path finish; the production adapters that followed the same day (Prompt 7B2) are documented in the next section.

- Server route `POST /api/enhanced-frame` (`src/app/api/enhanced-frame/route.ts`) is a thin transport wrapper. The whole contract — input validation, adapter selection, timeout, structured errors — lives in the pure core `src/features/enhanced-frame/server.ts` and is unit-tested directly without a running server.
- D027 single-active selection: the server-side `IMAGE_PROVIDER` setting (`mock` when unset, `seedream`, `wanxiang`) resolves to exactly one adapter via `selectImageGenerationAdapter` (`src/features/enhanced-frame/adapter-selection.ts`) with an injectable registry. In 7B1 the registry is empty, so a production setting answers a structured `501 provider_adapter_not_implemented` — never a silent mock fall-back (mock output must never be mistaken for provider output).
- Request contract (multipart/form-data), adopting the strictest documented provider limits from `docs/PROVIDER_SPIKE.md`: `compositionImage` (png/jpeg/webp, ≤20MB), optional `characterReferences` (≤9 files, same limits), optional `styleReference`, `prompt` (≤5000 chars).
- Response: `{ imageBase64, imageMimeType, adapterId, adapterVersion, provider, metadata }`. Failures are structured `{ error, message }`: 400/413 input errors, 500 provider misconfiguration, 501 not implemented, 502 adapter failure, 504 generation timeout (outer budget 120s; per provider HTTP attempt 100s — pro-tier image generation legitimately exceeds 30s, live-verified 2026-09-22).
- UI (`src/features/enhanced-frame/EnhancedFrame.tsx`): a collapsed-by-default explicit action. One click freezes the canonical ShotState, captures the current camera pose through the raw-export readiness gate, compiles the Prompt 6 deterministic generic prompt from the same frozen state, and sends composition + prompt to the route. Every failure is a local, retryable message; the raw export shares no state with this path and is never blocked (pinned by a browser test).
- Credentials: none in 7B1. `.env.example` documents the 7B2 conventions (`IMAGE_PROVIDER`, later `ARK_API_KEY` / `DASHSCOPE_API_KEY`), read only by the matching server-side adapter; values never reach the client, logs, fixtures or the prompt text.
- Content pipeline: `compile:content` now also emits `COMPILED_ADAPTER_CONFIGS` (validated, sorted, byte-deterministic) so the client compiles prompts through the content-pipeline-validated adapter config without a second drifting copy.

## Production image adapters (Prompt 7B2, 2026-09-22)

Both D027 providers are implemented server-side. Zero real provider calls were made in development or tests; the deterministic mock remains the default until the server-side `IMAGE_PROVIDER` selects a credentialed provider.

- Transport decision: plain REST over the Web `fetch` API — no vendor SDK (the Volcengine SDK is Python-only; Python is banned as a production runtime) and no OpenAI SDK (Ark's Seedream-specific fields do not fit its types, and one POST does not justify the dependency; consistent with the in-repo ZIP-writer precedent of keeping the dependency tree unchanged). Both providers share one injectable HTTP core, `src/adapters/image-generation/provider-http.ts`: per-attempt AbortController timeout, bounded retry (3 attempts, exponential backoff) on 429/5xx/network errors only — both providers bill successful outputs only — and caller-supplied Zod schemas so no provider payload is trusted beyond its declared contract.
- `SeedreamImageGenerationAdapter` (`src/adapters/image-generation/seedream.ts`, id `seedream@1.0.0`, the D027 primary): POSTs `{base}/images/generations` with Bearer `ARK_API_KEY`, model default `doubao-seedream-5-0-pro-260628` (the developer's activated Ark-console model, 2026-09-22; overridable via `SEEDREAM_MODEL`), images as data URLs with the composition FIRST (references then style follow), `response_format: b64_json`. Size is live-verified (2026-09-22, doubao-seedream-5-0-pro): the API requires an explicit `WIDTHxHEIGHT` or supported preset — the adapter derives `WIDTHxHEIGHT` from the composition PNG's own IHDR dimensions (1280x720 landscape / 720x1280 portrait, matching the Seedance 720p first-frame raster), falls back to `SEEDREAM_FALLBACK_SIZE` (1280x720) for unparseable compositions, and an explicit `SEEDREAM_SIZE` always wins; the metadata records which source produced the value. Base URL and model are equally constructor/env overridable. The account-side path is live-verified (endpoint + auth + structured error surfacing through the full 7B1 error contract).
- `WanxiangImageGenerationAdapter` (`src/adapters/image-generation/wanxiang.ts`, id `wanxiang@1.0.0`, the D027 backup): POSTs the official SYNCHRONOUS multimodal-generation endpoint (no `X-DashScope-Async`, no polling) with Bearer `DASHSCOPE_API_KEY`, model default `wan2.7-image`, size `2K`. Per the official rule that the output aspect ratio follows the LAST input image, the composition frame is placed LAST (references/style first). Result images are decoded inline from data URIs or downloaded when returned as URLs. Base URL defaults to the legacy `dashscope.aliyuncs.com` domain; workspace-dedicated domains are a `DASHSCOPE_BASE_URL` override.
- Registry & credentials (`src/features/enhanced-frame/registry.ts`): a production adapter registers ONLY when its credential (`ARK_API_KEY` / `DASHSCOPE_API_KEY`) is present and non-blank — the switch can never route an unauthenticated request. Selecting a production provider without its credential answers a structured `503 provider_credentials_missing` (added to the 7B1 error contract); `501 provider_adapter_not_implemented` remains only as the defensive case of a credentialed provider lacking a registered adapter.
- Contract tests (`tests/unit/{seedream,wanxiang}-adapter.test.ts`, `image-adapter-registry.test.ts`, `enhanced-frame-server.test.ts`): the `fetch` and `sleep` dependencies are fully faked, so the suite pins each adapter's request shape (URL, auth header, body fields, image ordering), response-Zod enforcement, retry/timeout behavior and determinism with zero network and zero cost. Provider-side behavior on real infrastructure remains deliberately untested until the developer authorizes live calls.
