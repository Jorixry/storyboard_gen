...
- Response: `{ imageBase64, imageMimeType, adapterId, adapterVersion, provider, metadata }`. Failures are structured `{ error, message }`: 400/413 input errors, 500 provider misconfiguration, 501 not implemented, 502 adapter failure, 504 generation timeout (default 30s).
