...
- `SeedreamImageGenerationAdapter` (`src/adapters/image-generation/seedream.ts`, id `seedream@1.0.0`, the D027 primary): POSTs `{base}/images/generations` with Bearer `ARK_API_KEY`, model default `doubao-seedream-4-0-250828`, images as data URLs with the composition FIRST (references then style follow), `response_format: b64_json`. Base URL, model and `size` (default `adaptive`) are constructor/env overridable. ⚠️ The Ark request-shape details are cross-validated (official doc page renders client-side; see docs/PROVIDER_SPIKE.md UNCONFIRMED items) and must be confirmed on the first authorized live call.
