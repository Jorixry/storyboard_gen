...
- Prompt 7B1 (mock-path finish + failure handling, Day 8 fallback branch) implemented 2026-09-22: server route `/api/enhanced-frame` with the pure handler core, D027 single-active adapter selection (mock default), explicit-action UI with retryable failures that never block raw export, `.env.example` conventions, and `compile:content` now emitting `COMPILED_ADAPTER_CONFIGS` so the client compiles the Prompt 6 generic prompt from the validated config. The new panel is collapsed in the controls aside, which the studio visual baselines do not capture (they scope the gallery and stage pane), so no baseline changed. Zero provider calls, zero credentials, zero cost.
- Prompt 7B2 (production adapters) implemented 2026-09-22: `SeedreamImageGenerationAdapter` (Ark REST, D027 primary) and `WanxiangImageGenerationAdapter` (DashScope sync REST, backup) over one shared injectable HTTP core (timeout, bounded 429/5xx retry, Zod response contracts; REST-over-SDK rationale in ARCHITECTURE). Registry registers a provider only when its credential exists; selecting a production provider without its key answers 503 `provider_credentials_missing`. All contract tests fake fetch/sleep — zero real provider calls, zero cost, credentials never committed. Open item: the Seedream request shape is cross-validated only (official doc page renders client-side); confirm on the first developer-authorized live call. Next packet: Prompt 8 (complete export package).
...
- Paid calls still require explicit per-call authorization from the developer; real provider behavior (Seedance D026 account-level checks, Seedream live request-shape confirmation) remains unverified until then.
